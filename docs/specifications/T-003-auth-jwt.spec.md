...
- FR-02: `POST /api/v1/auth/select-tenant` recibe `{ tenantId }` con
  ...
  (httpOnly, Secure, SameSite=Strict, Path=/api/v1/auth/refresh, TTL 7d).
...
- FR-07: `TenantContextFilter` (OncePerRequestFilter, corre después de
  ...
  lo usen.
- FR-08: El `SecurityConfig` debe dejar públicos: `POST /api/v1/auth/**`,
  `GET /actuator/health`. Todo lo demás requiere autenticación.
...
- PostgreSQL para refresh tokens (tablas V003 + V010).
